...
void main() {
  ...
  test('Ignore wild patterns', () async {
    ...
    File(path.join(tempDir.path, 'keep-file.txt')).writeAsStringSync('Keep this file');
    File(path.join(tempDir.path, 'react-app.js')).writeAsStringSync('React application');
    File(path.join(tempDir.path, 'normal.dart')).writeAsStringSync('Normal dart file');
    File(path.join(tempDir.path, 'test-utils.js')).writeAsStringSync('Test utilities');
    ...
    final output1 = File(path.join(tempDir.path, 'result1.txt')).readAsStringSync();
    ...
    final output2 = File(path.join(tempDir.path, 'result2.txt')).readAsStringSync();
